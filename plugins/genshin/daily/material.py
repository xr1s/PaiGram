...
class DailyMaterial(Plugin):
    ...
    daily_materials: List[Dict[str, "AreaDailyMaterials"]] = None  # type: ignore
    """
    daily_materials 储存的是一周中每天能刷的素材 ID
    按照如下形式组织
    ```python
    daily_materials[周几][国家] = AreaDailyMaterials(
      avatar=[角色, 角色, ...],
    ...
    async def initialize(self):
        ...
        async def task_daily():
            async with self.locks[0]:
                ...
                await self._refresh_daily_materials()
        ...
        if await aiofiles.os.path.exists(DATA_FILE_PATH):
            async with aiofiles.open(DATA_FILE_PATH) as cache:
                ...
                class Loader(BaseModel):
                    daily_materials: List[Dict[str, AreaDailyMaterials]]

                loader = Loader(daily_materials=json.loads(await cache.read()))
                self.daily_materials = loader.daily_materials
    ...
    async def daily_material(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE"):
        ...
        # 获取已经缓存的秘境素材信息
        if not self.daily_materials:  # 若没有缓存每日素材表的数据
            logger.info("正在获取每日素材缓存")
            await self._refresh_daily_materials()

        # 尝试获取用户已绑定的原神账号信息
        client, user_owned = await self._get_items_from_user(user)
        today_materials = self.daily_materials[weekday]
    ...
    async def refresh(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE"):
        ...
        async with self.locks[1]:  # 锁住第二把锁
            ...
            notice = await notice.edit_text(
                "每日素材表" + ("摘抄<b>完成！</b>" if self.daily_materials else "坏掉了！等会它再长好了之后我再抄。。。") + "\n正搬运每日素材的图标中。。。",
                parse_mode=ParseMode.HTML,
            )
    ...
    async def _refresh_daily_materials(self, retry: int = 5):
        """刷新来自 honey impact 的每日素材表"""
        for attempts in range(retry):
            try:
                res = await self.client.get("https://genshin.honeyhunterworld.com/?lang=CHS")
                ...
                continue
            self.daily_materials = _parse_honey_impact_source(res.content)
            content = json.dumps(
                self.daily_materials,
                ensure_ascii=False,
                separators=(",", ":"),
...
def _parse_honey_impact_source(source: bytes) -> List[Dict[str, "AreaDailyMaterials"]]:
    ...
    daily_materials: List[Dict[str, "AreaDailyMaterials"]] = [{} for _ in range(7)]
    ...
    for element in calendar:
        if element.tag == "span":  # 找到代表秘境的 span
            ...
            for div in element.iterfind("div"):  # div 对应的是一周中的每一天，一共 7 个 div
                ...
                if current_country not in daily_materials[weekday]:
                    daily_materials[weekday][current_country] = AreaDailyMaterials()
                materials = getattr(daily_materials[weekday][current_country], materials_type)
                for a in div:  # 当天能刷的所有素材在 a 列表中
                    href = typing.cast(str, a.attrib["href"])  # 素材的 ID 在 href 中
        ...
        if element.tag == "a":
            ...
            if item_name == "旅行者":
                continue
            href = typing.cast(str, element.attrib["href"])
            item_type = "weapon" if href.startswith("/i_n") else "avatar"
            item_id = "{}{}".format(
                "" if href.startswith("/i_n") else "10000",
                "".join(filter(str.isdigit, href)),
            )
            for weekday in map(int, element[0].attrib["data-days"]):
                getattr(daily_materials[weekday][current_country], item_type).append(item_id)
...
class AreaDailyMaterials(BaseModel):
    ...
    mastery 是排除旅行者后该国当日可以突破天赋的角色 ID 列表
    ...
    """
    mastery_materials 是当日该国所有可以刷的精通和炼武素材的 ID 列表
